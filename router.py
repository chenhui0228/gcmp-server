...
class Controller(Resource):
    def __init__(self, conf):
        ...
        if isinstance(conf.get('database'), dict):
            self.database_conf = conf['database']
        dtype = self.database_conf.get('type', 'sqlite')
        path = self.database_conf.get('sqlite.path', '/var/backup/backup.db')
        path = 'sqlite:///%s'%path
        self.db = db_api.get_db(type=dtype, path=path)
...
class APIRouter(object):
    ...
    def dispatch(self, req):
        ...
        if reqargs:
            ...
            raise HTTPError(405, 'method is not allowed')
